...
export class AggregatorUtils implements Utils {
  constructor(
    ...
    this._web3Instance = new Web3(
      this.provider || (globalThis as any)?.ethereum
    );
    ...
    if (walletConfig) {
      ...
      this._web3Instance.eth.accounts.wallet.add(
        this.walletConfig as WalletConfig
      );
    }
  ...
  sendSafeModeTransaction(transactionConfig: Partial<ethers.Transaction>) {
    ...
    this._web3Instance.eth
      .getTransactionCount(transactionConfig.from as string)
      .then(nonce => {
        ...
        const priorityFee = BigNumber.from(
          transactionConfig?.maxPriorityFeePerGas || 2000000000
        );
        this._web3Instance.eth.getGasPrice().then(gasPrice => {
          transactionConfig.maxFeePerGas =
            transactionConfig?.maxFeePerGas ||
            priorityFee.add(BigNumber.from(gasPrice));
          transactionConfig.maxPriorityFeePerGas =
  ...
  sendTransaction(transactionConfig: TransactionConfig) {
    if (
      ...
      transactionConfig?.from?.toString?.()?.toLowerCase?.() !==
        this.walletConfig?.address?.toLowerCase?.()
    ) {
      throw UtilsException.invalidParamError(
    ...
    this._web3Instance.eth
      ...
      .then(estimateGas => {
        ...
        if (typeof (globalThis as any)?.ethereum === 'object') {
          ...
          if (
            finalProvider?.providers &&
            (this._web3Instance.currentProvider as any)?.isMetaMask
          ) {
  ...
  async signMessage(message: string): Promise<string> {
    if ((globalThis as any).ethereum) {
      ...
      const signature = await this._web3Instance.eth.personal.sign(
        message,
        account,
        ''
      );
      ...
      // server side
      const signResult = this._web3Instance.eth.accounts.sign(
        message,
        this.walletConfig?.privateKey as string
      );
